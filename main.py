...
def visualize_qubit_states():
    """
    Creează și vizualizează stările |0>, |1>, |+> și |-> pe sfere Bloch separate.
    """
...
def create_alice_circuit(bits, bases):
    """
    Creează un circuit cuantic care encodează biții lui Alice folosind bazele specificate.

    Args:
        bits (list[int]): Șirul de biți pe care Alice vrea să-i trimită.
        bases (list[int]): Șirul de baze (0 pentru Z, 1 pentru X) folosite de Alice.

    Returns:
        QuantumCircuit: Circuitul cuantic cu qubiții preparați.
    """
    n = len(bits)
...
def measure_message(qc, bases):
    """
    Adaugă la circuit operațiile de măsurare ale lui Bob, conform bazelor sale.

    Args:
        qc (QuantumCircuit): Circuitul primit.
        bases (list[int]): Bazele alese de Bob pentru măsurare.

    Returns:
        QuantumCircuit: Circuitul complet, gata de a fi simulat.
    """
    n = len(bases)
    for i in range(n):
        if bases[i] == 1:
...
def run_e91_demo():
    """
    Simulează principiul de bază al protocolului E91 bazat pe inseparabilitate.
    """
...
def run_bb84_simulation(with_eve=False):
    """Funcție completă pentru a rula simularea BB84, cu sau fără spion."""
    KEY_LENGTH = 32
    ...
    if not with_eve:
        ...
        for i in range(KEY_LENGTH):
            ...
            measured_bit = int(list(result.get_counts().keys()))
    ...
    bob_measured_bits_str = list(result.get_counts().keys())
...
def main_menu():
    """Afișează meniul principal și gestionează alegerile utilizatorului."""
    while True:
